app.controller('fixedPaymentController',['$scope','$q','$filter','config','vehicleService','partyService','requestService','packageService',
    ...
    $scope.calculatePayment=function(){
        ...
        if(requestService.request.fixed){
            $scope.data=$filter('filter')(requestService.request.fixed,{"partyName":$scope.filter.party,"vehicle":$scope.filter.vehicle,"month":$scope.filter.month,"year":$scope.filter.year}); 
            $scope.loading=false;
            getPackageData();
        }else{
