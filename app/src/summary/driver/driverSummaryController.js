    ...
    function($scope,$state,$filter,$q,config,driverService,requestService,expenseService){
        ...
        loadSummary=function(){
           ...
           for(var i=0; i<monthSet.length;i++){
                chartData.local.push($filter('filter')(res[0].data,{'month':monthSet[i],'requestType':'local'}).length + $filter('filter')(res[1].data,{'month':monthSet[i],'requestType':'local'}).length);
                chartData.out.push($filter('filter')(res[0].data,{'month':monthSet[i],'requestType':'out'}).length + $filter('filter')(res[1].data,{'month':monthSet[i],'requestType':'out'}).length);
            }
                updateSummary();
        });
        },
